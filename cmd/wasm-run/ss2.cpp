...
uint32_t CallContract(void *addr , void *args, uint32_t len);
...
public:
	...
	void hii(NativeArg tt0) {
		printf("hello world steven \n");
...
void invoke()
{
	...
	size_t size = pack_size(tt) + pack_size(action);
	//size_t size = 68;
	printf("size %lu\n", size);
